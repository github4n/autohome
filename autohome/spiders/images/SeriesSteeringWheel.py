...
class SeriesImgsSpider(scrapy.Spider):
    ...
    def start_requests(self):
        for item in self.queryItems:
            url = 'https://car.autohome.com.cn/pic/series/%s-10.html' % item['id']
            PathUtils.createDir(self.rootPath + '/output//picture/' + item['name'])
            PathUtils.createDir(self.rootPath + '/output//picture/' + item['name']+'/中控方向盘')  # 创建目录
    ...
    def saveImage(self, response):
        ...
        rootPath = PathUtils.getRootPath()
        with open("%s/output/picture/%s/中控方向盘/%s" % (rootPath, carName, fileName), 'wb') as f:
...
if __name__ == "__main__":
    execute(['scrapy', 'crawl', 'SeriesSteeringWheelSpider'])
